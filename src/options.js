  ...
  if (document.querySelector("#message")) {
    ...
    setTimeout(() => {
      chrome.storage.local.set({ CharacterImg: TempImg }, function () {
        var status = document.getElementById("status-img");
...
function save_authorization() {
  //   alert(document.getElementById("authorization").value);
  if (!document.getElementById("authorization").value.match(/\S/g)) {
    alert("認証コードを指定してください。");
  } else {
    var authorization = document.getElementById("authorization").value;
    chrome.storage.local.set({ authorization: authorization }, function () {
      var status = document.getElementById("status-authorization");
      status.innerHTML = `<h5 style="margin-top: 20px;">設定を保存しました。</h5>`;
      setTimeout(function () {
        status.textContent = "";
      }, 5000);
...
document.getElementById("save-img").addEventListener("click", save_img);
document
  .getElementById("save-authorization")
  .addEventListener("click", save_authorization);
